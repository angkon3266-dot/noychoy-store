...
| 2026-08-30 | *this change* | **Two reported bugs.** Admin → Categories was 500ing on every load — the deleted product-template picker left three `config('theme.product_templates')` references in its view, and `@foreach(null)` is fatal; the dead markup is gone and a smoke test now renders every parameterless admin screen (the missing coverage that let it reach production). The homepage flashed plain text before the real page: that is the pre-hydration SEO shell React clears on mount, so it is now held at `opacity:0` for script-capable visitors and reveals itself after 1.2s only if the bundle never arrives — crawlers and JS-less visitors still get it verbatim. 718 tests |

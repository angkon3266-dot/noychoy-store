...
| 11 | **Move the support mailbox** | Admin → Settings (store email, mail from/username) and the Contact / Privacy pages | Customer-facing addresses are still `support@meridianeclat.com` because that is the mailbox that is actually read; switch them only once a `@noychoy.com` mailbox exists in Microsoft 365 (that is where `noychoy.com` MX points) |

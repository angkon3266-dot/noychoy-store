...
| Google product category blank on all 19 categories | **Todo** | owner | The code reads `categories.google_category`, walks up to the parent, then falls back to a store-wide default — but every row is null, so every item shows "Missing". Fixable in Admin → Categories (the picker already lists the 13 jewelry taxonomy nodes). The 6 products with no primary category need `META_GOOGLE_CATEGORY` or a category. Now costs twice over: the Google Shopping feed at `/feed/google.xml` reads the same field, and Merchant Center uses it to decide which searches an item can appear in |

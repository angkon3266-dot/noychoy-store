...
| 11 | **Paste the OpenAI key and switch the assistant on** | Admin → System Config → AI assistant (key, model `gpt-5-mini`, greeting), then "Test connection" | The chat assistant is built and deployed but stays hidden until a key is stored there; nobody in this repo can (or should) hold the key |

        ...
        <AccountShell wide>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
                ...
                <Link href="/account/loved" className="card p-4 hover:border-gold-300 transition">
                    <div className="text-2xl font-semibold text-red-500">{stats.loved}</div>
            ...
            {liveOffers.length > 0 && (
                <div className="card p-5 mb-8 border-gold-300">
                    <h2 className="font-semibold mb-3 flex items-center gap-2">🎁 Your exclusive offers <span className="badge bg-gold-600 text-white text-[10px]">{liveOffers.length}</span></h2>
                            ...
                            <div key={i} className="rounded-xl border-2 border-gold-300 bg-gradient-to-r from-gold-100/70 to-white p-3">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        ...
                                        <p className="text-xs text-green-700 mt-1">✓ Applied automatically at checkout{offer.until ? ` · until ${offer.until}` : ''}</p>
                                    </div>
            ...
            {open && (
                <div className="px-5 pb-5 space-y-5">
                    ...
                    <div className="rounded-lg bg-ink-50 border border-ink-100 p-3 text-xs text-ink-700/80 space-y-1">
                        <p>🛍️ Earn <strong>{loyalty.per1000} points</strong> for every <strong>৳1000</strong> you spend — added once your order is <strong>delivered</strong>. The more you buy, the more you save!</p>
                        <p>⭐ Write a review: <strong>+{loyalty.reviewPoints} points</strong> (+{loyalty.reviewPhotoBonus} with a photo)</p>
                        <p>💰 <strong>100 points = {loyalty.value100Text}</strong> — use them to cut your bill at checkout.</p>
                    </div>

                    {usage && (
                        <div className="rounded-xl border border-gold-200 bg-gold-50 p-4">
                            <h3 className="font-semibold text-sm flex items-center gap-2">💎 Your member discount ({usage.percent}% off)</h3>
                            {usage.capped ? (
                                usage.remaining > 0 ? (
                        ...
                        <ul className="space-y-2">
                            {loyalty.milestones.map((m, i) => (
                                <li key={i} className="flex items-center gap-3 text-sm">
                                    <span className={`w-7 h-7 grid place-items-center rounded-full ${m.done ? 'bg-green-100 text-green-700' : 'bg-ink-100 text-ink-400'}`}>
                                        {m.done ? '✓' : m.icon}
                                    </span>

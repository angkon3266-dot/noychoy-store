    ...
    return (
        <div className="mx-auto max-w-3xl px-4 py-10">
            ...
            <div className="space-y-3">
                {items.data.length === 0 && <div className="card p-10 text-center text-ink-700/70">No notifications yet.</div>}
                {items.data.map((n, i) => {
                    // Without a url there is nothing to activate, so this must
                    // not be a link: `pointer-events-none` hides it from the
                    // mouse but leaves it in the keyboard tab order.
                    const Tag = n.url ? 'a' : 'div';
                    return (
                        <Tag
                            key={i}
                            {...(n.url ? { href: n.url } : {})}
                            className={`card p-4 flex items-start gap-3 transition ${n.url ? 'hover:border-gold-300' : ''}`}
                        >
                            <span className="text-2xl shrink-0">{n.icon}</span>
                            <div className="min-w-0">
                                <p className="font-medium">{n.title}</p>
                                {n.body && <p className="text-sm text-ink-700/70 mt-0.5">{n.body}</p>}
                                <p className="text-xs text-ink-700/70 mt-1">{n.time}</p>

    ...
    return (
        <AuthCard title="Enter your code" subtitle="Check your SMS for the 6-digit code, then set a new password.">
            <form onSubmit={(e) => { e.preventDefault(); form.post('/password/reset', { ...keepOnErrors, onError: () => form.reset('password', 'password_confirmation') }); }} className="mt-6 space-y-4">
                <div><label className="label">6-digit code</label><input {...f('otp')} inputMode="numeric" maxLength={6} className="input tracking-widest text-center text-lg" placeholder="••••••" required autoComplete="one-time-code" /></div>
                <div><label className="label">New password</label><input {...f('password', 'password')} className="input" required minLength={6} autoComplete="new-password" /></div>
                <div><label className="label">Confirm new password</label><input {...f('password_confirmation', 'password')} className="input" required autoComplete="new-password" /></div>

    ...
    return (
        <AuthCard title="Choose a new password">
            <form onSubmit={(e) => { e.preventDefault(); form.post('/password/reset-email', { ...keepOnErrors, onError: () => form.reset('password', 'password_confirmation') }); }} className="mt-6 space-y-4">
                <div><label className="label">Email<input type="email" value={form.data.email} className="input" readOnly /></label></div>
                <div><label className="label">New password<input {...f('password', 'password')} className="input" required autoComplete="new-password" /></label></div>

            ...
            <Link href={p.url} className="block">
                ...
                {p.rating != null && (
                    <div className="mt-1 flex items-center gap-1 text-xs">
                        ...
                        <span className="text-ink-700/50">({p.review_count})</span>
                    </div>
                )}
                <div className="mt-1 flex items-center gap-2 flex-wrap">
                    {p.member ? (
                        <>
                            ...
                            <span className="text-xs text-ink-400 line-through">{p.price_text}</span>
                        ...
                        <>
                            ...
                            {p.on_sale && <span className="text-xs text-ink-400 line-through">{p.compare_text}</span>}
                        </>
                    )}
                </div>
            </Link>
            ...
            <div className="mt-2 transition duration-200 ease-out md:opacity-0 md:translate-y-1 md:pointer-events-none md:group-hover:opacity-100 md:group-hover:translate-y-0 md:group-hover:pointer-events-auto">
                {soldOut ? (
                    ...
                    <Link href={p.url} className="flex w-full items-center justify-center gap-1.5 rounded-full border border-ink-200 px-3 py-2 text-xs font-medium text-ink-800 hover:border-gold-400 hover:text-gold-700 transition">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M10.5 6h9.75M10.5 12h9.75m-9.75 6h9.75M3.75 6h.007v.008H3.75V6zm.375 6a.375.375 0 11-.75 0 .375.375 0 01.75 0zm-.375 6h.007v.008H3.75V18z" /></svg>
                        Select options
                    </Link>
                ) : (

...
<form method="GET" class="flex flex-wrap gap-2 mb-4">
    <input name="q" value="{{ request('q') }}" placeholder="Name, phone or email…" class="input py-2 w-64">
    ...
    <label class="flex items-center gap-1.5 text-sm px-2"><input type="checkbox" name="repeat" value="1" onchange="this.form.submit()" @checked(request('repeat'))> Repeat</label>
    <label class="flex items-center gap-1.5 text-sm px-2"><input type="checkbox" name="members" value="1" onchange="this.form.submit()" @checked(request('members'))> Members</label>
    <button class="btn-outline">Search</button>
    <a href="{{ route('admin.customers.import') }}" class="btn-outline ml-auto">⬆ Import CSV</a>
</form>

<div class="card overflow-hidden">
    <table class="w-full text-sm">
        <thead class="bg-ink-50 text-left text-xs uppercase tracking-wide text-ink-700/60">
            <tr><th class="px-4 py-3">Customer</th><th class="px-4 py-3">Orders</th><th class="px-4 py-3">Spent</th><th class="px-4 py-3">Last order</th><th class="px-4 py-3">Type</th></tr>
        </thead>
        <tbody class="divide-y divide-ink-100">
            ...
            @empty
                <tr><td colspan="5" class="px-4 py-10 text-center text-ink-700/50">No customers found.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>

...
<div class="min-h-screen flex">
    ...
    <aside class="fixed inset-y-0 left-0 z-40 w-60 bg-ink-900 text-gold-100/80 transform transition md:translate-x-0 flex flex-col"
        ...
        @php $nav = [
            ...
            ['landing.index','Landing pages','M3.75 3v11.25A2.25 2.25 0 006 16.5h12M3.75 3h-1.5m1.5 0h16.5m0 0h1.5m-1.5 0v11.25A2.25 2.25 0 0118 16.5h-2.25m-7.5 0h7.5M12 9v6m3-3H9'],
        ...
        @php $navUnreadMessages = \App\Models\ContactMessage::where('is_read', false)->count(); @endphp
            ...
            @foreach($nav as [$route, $label, $icon])
                ...
                <a href="{{ route('admin.'.$route) }}"
                   class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm {{ request()->routeIs('admin.'.\Illuminate\Support\Str::before($route,'.').'*') ? 'bg-white/10 text-white' : 'hover:bg-white/5' }}">
                    ...
                    {{ $label }}
                    @if($route === 'dashboard' && $navUnreadMessages > 0)
                        <span class="ml-auto min-w-[18px] h-[18px] px-1 rounded-full bg-red-600 text-white text-[10px] font-semibold inline-flex items-center justify-center" title="{{ $navUnreadMessages }} unread message(s)">{{ $navUnreadMessages > 9 ? '9+' : $navUnreadMessages }}</span>
                    @endif
                </a>
            @endforeach
        </nav>

...
<body class="min-h-screen flex flex-col" data-shop>
        ...
        <aside x-ref="panel" tabindex="-1"
            ...
            <div class="shrink-0 border-t border-ink-100 px-4 py-3 flex items-center justify-between" style="padding-bottom: calc(0.75rem + env(safe-area-inset-bottom))">
                ...
                <div class="flex items-center gap-2">
                    @foreach(\App\Support\SocialLinks::present() as $social)
                        <a href="{{ $social['url'] }}" target="_blank" rel="noopener" aria-label="{{ $social['label'] }}" class="w-9 h-9 grid place-items-center rounded-full bg-ink-50 text-ink-700/70 hover:text-gold-700">
                            <svg class="w-4.5 h-4.5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="{{ \App\Support\SocialLinks::PLATFORMS[$social['platform']]['icon'] }}"/></svg>
                        </a>
                    @endforeach
                </div>
            </div>
    ...
    <footer class="mt-16 bg-ink-900 text-gold-100">
        ...
        <div class="mx-auto max-w-7xl px-4 py-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            <div>
                ...
                @php $footerSocials = \App\Support\SocialLinks::present(); @endphp
                @if($footerSocials)
                    <div class="mt-4 flex gap-3">
                        @foreach($footerSocials as $social)
                            <a href="{{ $social['url'] }}" target="_blank" rel="noopener" class="text-gold-100/70 hover:text-white" aria-label="{{ $social['label'] }}"><svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="{{ \App\Support\SocialLinks::PLATFORMS[$social['platform']]['icon'] }}"/></svg></a>
                        @endforeach
                    </div>
                @endif
            </div>
    ...
    @php
        ...
        $memberUsage = null;
        if (auth('customer')->check() && member_pricing()->enabled()) {
            $memberUsage = member_pricing()->usageStatus(auth('customer')->user());
        }
        $hasMemberDiscount = $memberUsage && $memberUsage['percent'] > 0;
        $showOffersButton = $memberOffers->isNotEmpty() || $hasMemberDiscount;
    @endphp
    @php
        $assistantOn = app(\App\Services\Ai\AssistantService::class)->enabled();
    @endphp
    @if($assistantOn || $showOffersButton || theme('show_call_button') || (theme('show_whatsapp_button') && theme('whatsapp_number')) || (theme('show_messenger_button') && theme('messenger_url')))
        <div class="fixed right-5 z-50 flex flex-col items-center gap-3 bottom-[calc(4.75rem_+_env(safe-area-inset-bottom))] md:bottom-5">
            {{-- Chat assistant — the panel is partials.ai-chat; this only opens it --}}
            @if($assistantOn)
                <button type="button" onclick="window.NoyChat && window.NoyChat.toggle()" data-noy-chat-launcher
                        class="flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full bg-ink-900 text-white shadow-lg hover:scale-105 transition p-3.5"
                        title="Ask us anything" aria-label="Ask us anything">
                    <svg class="w-full h-full" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M20.25 8.511c.884.284 1.5 1.128 1.5 2.097v4.286c0 1.136-.847 2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354 0-2.694-.055-4.02-.163a2.115 2.115 0 01-.825-.242m9.345-8.334a2.126 2.126 0 00-.476-.095 48.64 48.64 0 00-8.048 0c-1.131.094-1.976 1.057-1.976 2.192v4.286c0 .837.46 1.58 1.155 1.951m9.345-8.334V6.637c0-1.621-1.152-3.026-2.76-3.235A48.455 48.455 0 0011.25 3c-2.115 0-4.198.137-6.24.402-1.652.213-2.76 1.66-2.76 3.235v6.226c0 1.621 1.152 3.026 2.76 3.235.577.075 1.157.14 1.74.194V21l4.155-4.155"/></svg>
                </button>
            @endif
            {{-- Your offers --}}
            @if($showOffersButton)
                <div x-data="{ open: false }" class="relative">
                    <button type="button" @click="open = !open"
                        ...
                        @php
                            $offerBadge = $memberOffers->count() + ($hasMemberDiscount ? 1 : 0);
                        @endphp
                        <span class="absolute -top-1 -right-1 min-w-[20px] h-5 px-1 flex items-center justify-center rounded-full bg-red-600 text-white text-[11px] font-semibold">{{ $offerBadge }}</span>
                    </button>
                    <div x-show="open" x-cloak x-transition @click.outside="open = false"
                        ...
                        @if($hasMemberDiscount)
                            <div class="rounded-lg border border-gold-300 bg-gold-600 text-white p-2.5 mb-2">
                                <p class="text-sm font-medium">💎 {{ rtrim(rtrim(number_format($memberUsage['percent'], 2), '0'), '.') }}% member discount</p>
                                @if($memberUsage['capped'])
                                    @php
                                        $wmReset = $memberUsage['resets_at'] ? ' · resets '.$memberUsage['resets_at']->format('d M') : '';
                                    @endphp
                                    <p class="text-[11px] text-white/85 mt-0.5">
                                        {{ $memberUsage['remaining'] > 0 ? $memberUsage['remaining'].' of '.$memberUsage['max'].' uses left'.$wmReset : 'Used up'.$wmReset }}
                                    </p>
                                @else
                                    <p class="text-[11px] text-white/85 mt-0.5">Auto-applied on every order</p>
                                @endif
                            </div>
                        @endif
                        <div class="space-y-2">
